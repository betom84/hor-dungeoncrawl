...
impl Templates {
    pub fn load() -> Self {
        let file = File::open("resources/template.ron").expect("failed to open game template file");
        from_reader(file).expect("failed to load game template")
    }
        ...
        if let Some(effects) = &template.provides {
            effects
                ...
                .for_each(|(name, value)| match name.as_str() {
                    ...
                    _ => println!("unknown effect {} defined in game template", name),
                });
        }
    }
}
